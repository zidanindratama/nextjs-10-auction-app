...
import { usercColumns } from "./UserDataDependencies";
...
const UserDataTable = () => {
  ...
  const { data, isLoading, isSuccess, refetch, isRefetching } = useFetchData({
    ...
    dataProtected: `users?pgNum=${page}&pgSize=${count}&name=${searchValue}`,
    keep: true,
  });
    ...
    <Suspense>
      <div className="p-4 sm:px-6 sm:py-0">
        ...
        {isSuccess && !isRefetching && (
          <>
            <DataTable
              propsData={data?.data.users}
              columnsData={usercColumns}
              pageCount={pageCount}
            >
